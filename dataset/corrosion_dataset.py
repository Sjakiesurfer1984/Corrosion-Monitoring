...
# --- Color-to-ID Mapping ---
# This dictionary maps the RGB colour of a mask pixel to its corresponding class ID.
# This makes the code more robust and easier to manage.
COLOR_TO_ID = {
    (0, 0, 0): 0,         # Background
    (0, 255, 0): 1,       # Mild Corrosion (Green)
    (255, 255, 0): 2,     # Middl Corrosion (Yellow)
    (255, 0, 0): 3,       # Severe Corrosion (Red)
}
...
class CorrosionDataset(Dataset):
    ...
    def __init__(
        ...
        """
        The constructor for our dataset.
        ...
        # We need a reverse map for the numpy-based conversion.
        self.rgb_to_id_map = np.array([list(k) for k in COLOR_TO_ID.keys()])
        self.id_to_rgb_map = np.array([v for v in COLOR_TO_ID.values()])
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Loads and returns one sample from the dataset at the given index 'idx'.
        """
        ...
        # This is the improved logic using a predefined map.
        mask_np = np.array(mask)
        label_mask = np.zeros(mask_np.shape[:2], dtype=np.uint8)

        # Loop through the colour map to efficiently assign class IDs.
        for color, class_id in COLOR_TO_ID.items():
            # Create a boolean mask where all three colour channels match the current color.
            # Using `.all(axis=-1)` is a robust way to do this.
            match_mask = (mask_np == np.array(color)).all(axis=-1)
            # Apply the class ID to all matching pixels.
            label_mask[match_mask] = class_id
...
# --- Augmentation Classes (remain the same) ---
